...
createPicture images paddle ball blocks score =
  ...
  in pictures $ [getPicLeftTop "background",paddle `pictureTranslated` getPicLeftTop "paddle",ball `pictureTranslated` getPicLeftTop "ball"] <> ((`pictureTranslated` getPicLeftTop "block") <$> blocks) <> [renderScore images score]
...
accumB :: a -> EvStream (a -> a) -> Behavior a
accumB = undefined

setupNetwork :: forall a unit float. EvStream WE.Event -> EvStream TickData -> SurfaceMap a -> Behavior (Picture unit float)
setupNetwork eevent etick surfaces =
  let
    mouseXMovement :: Event Point
    mouseXMovement = fmap (fmap fromIntegral) (catMaybesEs ((mapped . _y .~ 0) . (^? WE._MouseAxis . WE.mouseAxisDelta) <$> eevent))
    ballBorderCollision :: EvStream CollisionDirection
    ballBorderCollision = catMaybesEs ((detectCollisionBorder <$> (ballRect <$> ballPosition)) <$ etick)
    ballPaddleCollision :: EvStream CollisionData
    ballPaddleCollision = catMaybesEs ((detectCollisionRect <$> (paddleRect <$> paddlePosition) <*> (ballRect <$> ballPosition)) <$ etick)
    blocks :: Behavior [Point]
    blocks = accumB initialBlocks ((\(collisionIdx,_) -> deleteNth collisionIdx) <$> ballBlockCollision)
    ballBlockCollision :: EvStream (Int,CollisionData)
    ballBlockCollision = catMaybesEs ((ballBlocksCollision <$> blocks <*> ballPosition) <$ etick)
    deltaVel :: Point -> TickData -> Point
    deltaVel v td = (realToFrac (toSeconds (td ^. currentDelta))) *^ v
    ballPosition :: Behavior Point
    ballPosition = accumB initialBallPosition ((+) <$> (deltaVel <$> ballVelocity <*> etick))
    ballBorderAndBlockCollision :: EvStream CollisionDirection
    ballBorderAndBlockCollision = ballBorderCollision <> (view (_2 . cdDirection) <$> ballBlockCollision)
  ...
  in
    (createPicture <$> pure surfaces <*> paddlePosition <*> ballPosition <*> blocks <*> score)

main :: IO ()
main = do
  withPlatform (WindowTitle "arkanoid, frp style") (ConstantWindowSize 640 480) MouseGrabYes $ \platform -> runNowMaster $ do
    (tickFire,tickCallback) <- callbackStream
    (eventFire,eventCallback) <- callbackStream
    md <- liftIO (readMediaFiles (loadImage platform) "media/images")
    initialTicks <- liftIO $ getTicks
    let pictureBehavior = pure mempty
    mainLoop initialTicks (pollEvents platform) (wrenchRender platform (md ^. mdSurfaces) (error "no font specified") (Just colorsBlack)) (\e -> liftIO $ eventCallback e) (\e -> liftIO $ tickCallback e) pictureBehavior

mainLoop :: TimeTicks -> IO [WE.Event] -> (Picture unit float -> IO ()) -> (WE.Event -> Now ()) -> (TickData -> Now ()) -> Behavior (Picture unit float) -> Now b
mainLoop oldTicks pollEvents' renderPicture eventCallback tickCallback currentPicture = do
  ticks <- liftIO $ getTicks
  events <- liftIO $ pollEvents'
  mapM_ eventCallback events
  tickCallback (TickData{_currentTicks=ticks,_currentDelta=(ticks `tickDelta` oldTicks)})
  currentPictureSample <- sampleNow currentPicture
  liftIO $ renderPicture currentPictureSample
  sync (threadDelay (fromSeconds (1/60)))
  mainLoop ticks pollEvents' renderPicture eventCallback tickCallback currentPicture
{-
module Main where
